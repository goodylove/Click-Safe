...
const mailParser = createTool({
  id: "email-parser",
  description: "Parses raw or pasted email text to extract sender, subject, links, headers, and domain/auth info.",
...
function analyzeHeaders(headers) {
  const find = (key) => Object.keys(headers).find((k) => k.toLowerCase() === key.toLowerCase());
  ...
  const fails = [spf, dkim, dmarc].filter(
    (v) => ["fail", "softfail", "neutral"].includes(v)
  );
  return {
    spf,
    dkim,
    ...
    reason: fails.length > 0 ? `Authentication failed: ${fails.join(", ")}` : "All authentication checks passed"
  };
}
