...
(() => {
  ...
  const getMainElement = () => {
    for (const sel of MAIN_SELECTORS) {
      try {
        ...
        if (el && isVisible(el) && (el.textContent || "").trim().length > 80)
          return el;
      } catch {}
    }
  ...
  // Cleanup - removing scripts, normalises whitespace
  ...
  // Link extraction with basic heuristics
  const extractLinks = () => {
    const result = [];
    ...
    for (const a of links) {
      ...
      try {
        ...
        if (/\.(tk|ml|ga|cf|gq|xyz|top)$/.test(u.hostname))
          factors.push("free_domain");
  ...
  const extractMetadata = () => {
    ...
    try {
      ...
      if (ctx.src === "OUTLOOK") {
        const senderName =
          document.querySelector('[data-testid="senderName"]')?.textContent ||
          "";
        const senderMail =
          document.querySelector('[data-testid="senderEmail"]')?.textContent ||
          "";
        const subj =
          document.querySelector('[data-testid="subject"]')?.textContent || "";
  ...
  const buildPayload = () => {
    ...
    try {
      mainEl = getMainElement();
    } catch (e) {
      errors.push("main_el");
      mainEl = document.body;
    }
    try {
      text = cleanText(mainEl as HTMLElement);
    } catch (e) {
      errors.push("clean_text");
    }
    try {
    ...
    const payload = {
      ...
      html,
      confidence:
        text.length > 500 ? "HIGH" : text.length > 150 ? "MEDIUM" : "LOW",
